...
contract YourContract is Ownable {
    ...
    uint public _minGiftValue;
    uint public _maxGiftValue;
    uint8 public _maxParticipants;
    address[] public _participants;
    mapping(address => uint32) _order;
    
    constructor(uint minGiftValue, uint maxGiftValue, bool privateGame, uint8 maxParticipants) {
        _minGiftValue = minGiftValue;
        _maxGiftValue = maxGiftValue;
        _privateGame = privateGame;
        _maxParticipants = maxParticipants;
        emit GameCreated(minGiftValue, maxGiftValue, privateGame, maxParticipants, block.timestamp);
    }

    event GameCreated(uint minGiftValue, uint maxGiftValue, bool privateGame, uint8 maxParticipants, uint timestamp);
    ...
    function approveSpendToken(address tokenAdress, uint _amount) public returns(bool){
    ...
    function depositTokens (address tokenAdress, uint256 _amount) external payable {
        ERC20interface = ERC20(tokenAdress);
    ...
    function getPrice() private view returns (int) {
...
*/
    ...
    function addParticipants(address[] memory participants) public onlyOwner {
     require(participants.length+_participants.length <= _maxParticipants, "Too many participants added");
    ...
    function shuffleParticipants() private onlyOwner {
      for (uint256 i = 0; i < _participants.length; i++) {
          uint256 n = i + uint256(keccak256(abi.encodePacked(block.timestamp))) % (_participants.length - i);
    ...
    function startGame() public onlyOwner returns (address[] memory) {
        require(_participants.length == _maxParticipants, "The game is not full");
        shuffleParticipants();
        wrapGifts();
